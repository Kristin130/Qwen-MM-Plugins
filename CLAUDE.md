...
9. **qwen-mm-plugins-cua** — First-party stdio MCP proxy for Cua Driver: resolves and delegates to the locally installed driver while retaining the Qwen plugin name; needs Cua Driver + a real desktop. `src/capabilities/cua/`.
...
│   ├── cua/                                 # CUA proxy capability (skill + stdio JSON-RPC proxy → external Cua Driver)
...
**App hosts (optional)** — blender/freecad live sessions + edu-agent rendering + CUA driver resolution. Full catalog: `src/shared/env.py` `CONFIG_FIELDS` (regenerate these tables via `python3 scripts/gen_env_docs.py`).
...
| `QWEN_MM_CUA_DRIVER_PATH` | cua | Absolute external Cua Driver path for the first-party proxy |
