...
command. This module is the shared, stdlib-only plumbing those launchers compose:
  ...
  - autolaunch_display_hint        — the one prerequisite auto-download can't satisfy (xvfb needs root)
  - wrap_xvfb                      — run a GUI app on a headless box under a virtual display
...
log = logging.getLogger("qwen-mm-plugins.applaunch")
...
# this can NOT satisfy is a display: xvfb needs root to install (see autolaunch_display_hint).
...
def apps_cache_dir() -> Path:
    ...
    demand). Shares shared.cache.cache_dir(), so QWEN_MM_CACHE relocates it too."""
...
def download_file(url: str, dest: Path, *, timeout: float = 60.0) -> None:
    ...
    pip deps). Writes to a `.part` sidecar and renames on success, so a partial download never looks
    complete. Logs coarse progress to stderr (large builds take a while)."""
    import urllib.request

    dest.parent.mkdir(parents=True, exist_ok=True)
    tmp = Path(str(dest) + ".part")
    req = urllib.request.Request(url, headers={"User-Agent": "qwen-mm-plugins"})
    with urllib.request.urlopen(req, timeout=timeout) as resp:  # noqa: S310 (pinned https app URL)
...
def autolaunch_display_hint() -> str | None:
    """Actionable hint when a headless autolaunch can't get a display, else None.

    autolaunch always starts the app headless (under xvfb), so a missing `xvfb-run` blocks it even on
    a box that happens to have a real $DISPLAY. This is the single step auto-download can't do for the
    user — installing xvfb needs root."""
...
def wrap_xvfb(cmd: list[str], *, screen: str = "1920x1080x24") -> list[str]:
    """Prefix `cmd` with `xvfb-run` so a GUI app runs on a headless box under a virtual display.
    Raises FileNotFoundError (with an install hint) if xvfb-run isn't available."""
...
def default_log_path(name: str) -> Path:
    ...
    Routes through shared.cache.cache_dir() like apps_cache_dir(), so QWEN_MM_CACHE relocates the
...
#
# A thin-client loader connects to a *running* app. When QWEN_MM_AUTOLAUNCH is set it may bring the
...
def launch_and_probe(host: str, port: int, launch_fn, *, app: str) -> bool:
    ...
    log.warning("QWEN_MM_AUTOLAUNCH set — launching %s + the bundled addon on %s:%s …", app, host, port)
    try:
...
def startup_probe(host: str, port: int, *, app: str, entry: str) -> None:
    ...
    if is_port_open(host, port):
        ...
        log.warning(
            "%s not reachable on %s:%s — QWEN_MM_AUTOLAUNCH is set, so it will be started on the first tool call.",
            app,
            host,
            port,
        )
        ...
        log.warning(
            ...
            "or set QWEN_MM_AUTOLAUNCH=1 (see SKILL.md).",
            app,
            host,
            port,
            entry,
